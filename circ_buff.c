...
{
tCircularBuffer *pTempBuff = (tCircularBuffer*)calloc(1, sizeof(tCircularBuffer));

    if (pTempBuff) {
        pTempBuff->pucBufferStartPtr = (unsigned char*)calloc(1,ulBufferSize);
...
//*****************************************************************************
void InitCircularBuffer(tCircularBuffer *pCircularBuffer)
{
    if (pCircularBuffer) {
                ...
                unsigned int uiPacketSize)
{
    ...
    int uiOffset = ((pCircularBuffer->pucWritePtr + uiPacketSize) - pCircularBuffer->pucBufferEndPtr);

    if (IsBufferVacant(pCircularBuffer,pCircularBuffer->ulBufferSize)) {
        if(uiOffset <= 0) {
            memcpy(pCircularBuffer->pucWritePtr, pucBuffer, uiPacketSize);
        } else {
            memcpy(pCircularBuffer->pucWritePtr, pucBuffer, (uiPacketSize - uiOffset));
            memcpy(pCircularBuffer->pucBufferStartPtr, ((pucBuffer + uiPacketSize) - uiOffset), uiOffset);
        }
...
// return none.
//*****************************************************************************
void UpdateReadPtr(tCircularBuffer *pCircularBuffer, unsigned int uiDataSize)
{

    int uiOffset = ((pCircularBuffer->pucReadPtr + uiDataSize) - pCircularBuffer->pucBufferEndPtr);
    if (uiOffset <= 0) {
        pCircularBuffer->pucReadPtr = (pCircularBuffer->pucReadPtr + uiDataSize);
        if (pCircularBuffer->pucReadPtr == pCircularBuffer->pucBufferEndPtr) {
            pCircularBuffer->pucReadPtr = pCircularBuffer->pucBufferStartPtr;
        }
    } else {
        pCircularBuffer->pucReadPtr = (pCircularBuffer->pucBufferStartPtr + uiOffset);
    }
...
// return none.
//*****************************************************************************
void UpdateWritePtr(tCircularBuffer *pCircularBuffer, unsigned int uiPacketSize)
{

    int uiOffset = ((pCircularBuffer->pucWritePtr + uiPacketSize) - pCircularBuffer->pucBufferEndPtr);
    if (uiOffset <= 0) {
        pCircularBuffer->pucWritePtr = (pCircularBuffer->pucWritePtr + uiPacketSize);
        if (pCircularBuffer->pucWritePtr == pCircularBuffer->pucBufferEndPtr) {
            pCircularBuffer->pucWritePtr = pCircularBuffer->pucBufferStartPtr;
        }
    } else {
        pCircularBuffer->pucWritePtr = (pCircularBuffer->pucBufferStartPtr + uiOffset);
    }
}
//*****************************************************************************
                ...
                unsigned int uiDataSize)
{
    ...
    if (uiOffset <= 0) {
        ...
        memcpy(pucBuffer, pCircularBuffer->pucReadPtr, (uiDataSize - uiOffset));
        memcpy(((pucBuffer + uiDataSize) - uiOffset), pCircularBuffer->pucBufferStartPtr, uiOffset);
    }
...
//*****************************************************************************
long FillZeroes(tCircularBuffer *pCircularBuffer, unsigned int uiPacketSize)
{
